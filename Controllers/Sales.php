<?php
	class Sales extends Controller
	{
		...
		public function addDetails()
		{
		    ...
		    $data['total'] = $this->model->totalVenta($id);
		    echo json_encode($data, JSON_UNESCAPED_UNICODE);
		}
		...
		{
			$id = $_SESSION['user_id'];
			$total = $this->model->totalVenta($id);
			$data = $this->model->register($total['total']);
			if ($data == "ok") {
				$details = $this->model->getDetails($id);
				$id_sales = $this->model->id_sales();
				foreach ($details as $detail) {
					$id_product = $detail['id_product'];
					$stock = $detail['stock'];
					$price = $detail['price'];
					$sub_total = $price * $stock;
					$this->model->registraDetail($id_sales['id'], $id_product, $stock, $price, $sub_total);
				...
				if ($vaciar == "ok") {
					$msg = array('msg' => 'ok', 'id_venta' => $id_sales['id']);
				}
			} else {
		...
		public function generarPdf($id_sales)
		{
			$empresa = $this->model->getEmpresa();
			$productos = $this->model->getProSales($id_sales);
		...
		{
			$data = $this->model->historialVenta();
			for ($i=0; $i < count($data) ; $i++) {
				$data[$i]['acciones'] = '<div>
					<a class="btn btn-danger" href="'.BASE_URL . "Sales/generarPdf/" . $data[$i]['id'] .'" target="_blank"><i class="fas fa-file-pdf"></i></a>
				</div>';
			}
			echo json_encode($data, JSON_UNESCAPED_UNICODE);
			die();
